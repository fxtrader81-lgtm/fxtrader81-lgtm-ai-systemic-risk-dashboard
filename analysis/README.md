...
- 看板第07页的历史事件颜色现在按**事件当月月末**数据重建，表示事中确认。后续六个月最大回撤以事件月最后交易日标普500收盘价为基准，取随后六个月内最低日收盘价除以基准再减1；若未跌破基准显示0%，缺少日线则显示N/A。结果不参与事件当月评分。第06页分别以事件前6个月和前3个月作信贷压力代理回放；两套事件口径不可合并计算命中率。
...
- 第07页顶部以SAFE、WATCH、WARNING、CRITICAL四级评级和评分为主，展示六项指标权重及汇总公式；市场阶段是辅助解释：尚未出现市场传导、有传导但市场尚未共振、指标共振风险释放中、风险压力消退市场恢复中。“压力消退”要求近三个月曾有WARNING以上，且当前四级评分回到SAFE、股票三个月动量非负、VIX环比回落、STLFSI不高于零。总览另显示05结构、06信贷、07市场、风险释放四个当前状态节点。阶段是探索性的状态快照，**不是经过样本外验证的顺序预测模型**。
