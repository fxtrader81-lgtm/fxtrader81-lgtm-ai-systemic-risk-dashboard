...
页面:
  1. 📊 Overview     — 归一化全资产对比 + 股灾标注
  2. 🇺🇸 US Market   — 美债山形 + 三大股指折线 双Y轴
  3. 🇨🇳 CN Market   — 美债山形 + 上证/深证折线 双Y轴
  4. 🚨 Alert System — 4指标预警卡片 + 历史CRITICAL复盘
...
.h-score { font-size: 13px; font-weight: 700; width: 50px; text-align: right; }
...
/* 来源标签 */
.src { display: inline-block; background: rgba(96,165,250,0.1); border: 1px solid rgba(96,165,250,0.2); border-radius: 5px; padding: 1px 8px; font-size: 11px; color: #60a5fa; margin-left: 6px; }
.src-y { display: inline-block; background: rgba(251,191,36,0.1); border: 1px solid rgba(251,191,36,0.2); border-radius: 5px; padding: 1px 8px; font-size: 11px; color: #fbbf24; margin-left: 6px; }

#MainMenu { visibility: hidden; } footer { visibility: hidden; } header { visibility: hidden; }
.modebar { display: none !important; }
.stTabs [data-baseweb="tab-list"] { background: transparent; gap: 4px; }
.stTabs [data-baseweb="tab"] {
    background: #0b1120; border-radius: 8px; padding: 8px 20px;
    color: #64748b; font-weight: 600; border: 1px solid rgba(255,255,255,0.07);
}
...
def fetch_yf_index(ticker: str, start: str = "1994-01-01") -> pd.Series:
    """从yfinance获取指数月度收盘价（A股 fallback）"""
    try:
...
def compute_alert_metrics(y10: pd.Series, y30: pd.Series, sp500: pd.Series) -> dict:
    ...
    # --- 指标② 10Y收益率3个月变化速率（最关键）---
    # 历史统计: P75=+50bps, P90=+100bps, P95=+150bps
    if len(y10) >= 3:
        chg3m = float(y10.iloc[-1] - y10.iloc[-3]) * 100  # 转bps
    else:
        chg3m = 0.0
    if chg3m < 50:
    ...
    if not y30.empty and not y10.empty:
        ...
        if len(common) > 0:
            spread = float(y30.loc[common[-1]] - y10.loc[common[-1]]) * 100  # bps
        else:
            spread = 20.0
    else:
        spread = 20.0
...
def add_crash_annotations(fig, events, y_ref=None, secondary=False):
    """在图表上添加股灾标注竖线"""
    for ev in events:
...
def build_overview_chart(y10, y30, sp500, nasdaq, dow, shcomp, szcomp, period, show_crashes):
    ...
    assets = [
        (y10,    "10Y Treasury",  "#5CB85C", "山形"),
        (y30,    "30Y Treasury",  "#5878FF", "山形"),
        (sp500,  "S&P 500",       "#4F8EF7", "线"),
        (nasdaq, "NASDAQ 100",    "#FF7F50", "线"),
        (dow,    "Dow Jones",     "#2CA58D", "线"),
        (shcomp, "上证指数",      "#E63946", "线"),
        (szcomp, "深证成指",      "#9D4EDD", "线"),
    ]

    for series, name, color, chart_type in assets:
        if series.empty:
            continue
...
def build_dual_axis_chart(y10, y30, *stock_series_names, period, show_crashes, title):
    """双Y轴图: 债券山形（左）+ 股指折线（右）"""
    fig = make_subplots(specs=[[{"secondary_y": True}]])

    BOND_COLORS = {"10Y Treasury": "#5CB85C", "30Y Treasury": "#5878FF"}
    STOCK_COLORS = {
        "S&P 500":   "#4F8EF7",
        "NASDAQ 100":"#FF7F50",
        "Dow Jones": "#2CA58D",
    ...
    for series, name in [(y10, "10Y Treasury"), (y30, "30Y Treasury")]:
        ...
        hex_col = BOND_COLORS[name]
        fig.add_trace(go.Scatter(
            x=s.index, y=s.values, name=name,
            fill="tozeroy",
            fillcolor=hex_col + "26",
    ...
    # 股指（折线，右轴）
    for series, name in stock_series_names:
        if series.empty:
            continue
        s = filter_by_period(series, period).dropna()
        color = STOCK_COLORS.get(name, "#94a3b8")
        fig.add_trace(go.Scatter(
            x=s.index, y=s.values, name=name,
...
def build_alert_history_chart(y10, y30, sp500, period):
    """预警页：历史CRITICAL事件标注在收益率图上"""
    fig = make_subplots(specs=[[{"secondary_y": True}]])
    ...
    if not y10.empty:
        ...
        fig.add_trace(go.Scatter(
            ...
            fill="tozeroy", fillcolor="#5CB85C26",
...
with st.sidebar:
    ...
    page = st.radio(
        "导航",
        ["📊 Overview", "🇺🇸 US Market", "🇨🇳 CN Market", "🚨 Alert System"],
        label_visibility="collapsed",
    )

    st.markdown("---")
    st.markdown('<div style="font-size:11px; color:#475569; margin-bottom:8px;">时间范围</div>', unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div style="font-size:11px; color:#475569; margin-top:12px; line-height:1.8;">
    数据源<br>
    <span style="color:#60a5fa;">FRED</span> · 美国国债收益率<br>
    <span style="color:#60a5fa;">FMP</span> · 美股三大指数<br>
    <span style="color:#fbbf24;">yfinance</span> · A股指数<br><br>
...
PAGE_TITLES = {
    "📊 Overview":      ("📊 全资产宏观走势", "归一化对比 · 股灾标注 · 倒挂区间"),
    "🇺🇸 US Market":   ("🇺🇸 美国市场", "美债收益率 × 三大股指 · 双Y轴原始点位"),
    "🇨🇳 CN Market":   ("🇨🇳 中国市场", "美债收益率 × 上证/深证 · 双Y轴原始点位"),
    "🚨 Alert System": ("🚨 预警系统", "基于历史统计分位数的4指标综合预警"),
}

ptitle, psub = PAGE_TITLES.get(page, ("Straw 6", ""))
now_str = datetime.now().strftime("%Y-%m-%d %H:%M ET")

st.markdown(f"""
<div style="display:flex; justify-content:space-between; align-items:flex-start; margin-bottom:20px;">
  <div>
    <div class="main-title">{ptitle}</div>
    <div class="sub-title">{psub}</div>
  </div>
...
if page == "📊 Overview":
    ...
    kpi_card(c1, "10Y Treasury", y10_now, "美国10年期国债收益率", "green" if not y10.empty and y10.iloc[-1] < 4 else "orange")
    kpi_card(c2, "30Y Treasury", y30_now, "美国30年期国债收益率", "green" if not y30.empty and y30.iloc[-1] < 4.5 else "orange")
...
# =========================================================
    ...
    <div class="panel">
      ...
      <div style="display:grid; grid-template-columns:1fr 1fr; gap:16px;">
        ...
        <div>
          <div style="font-size:13px; font-weight:700; color:#5CB85C; margin-bottom:8px;">🏔️ 山形图（收益率）× 折线图（股指）观察重点</div>
          <div style="font-size:13px; color:#94a3b8; line-height:1.8;">
            <b style="color:#fbbf24;">红色虚线区域</b>：30Y-10Y倒挂，历史上先于衰退出现<br>
            <b style="color:#ef4444;">收益率峰值</b>：通常在加息末期，股市往往同期底部<br>
            <b style="color:#4F8EF7;">2022年</b>：收益率+230bps，标普跌25%，最典型的负相关案例
          </div>
        </div>
      </div>
    </div>
...
# =========================================================
    ...
    <div class="alert-master-card">
      <div>
        ...
        <div class="bar-wrap">
          <div class="bar-fill" style="width:{bar_pct}%; background:{master_color};"></div>
        </div>
      </div>
    ...
    def alert_metric_card(col, icon, title, value, unit, grade, weight, threshold_html):
        ...
        with col:
            ...
            <div class="metric-card" style="height:210px;">
              ...
              <div>
                <span class="metric-badge" style="background:{color}22; color:{color}; border:1px solid {color}44;">{grade}</span>
              </div>
    ...
    alert_metric_card(
        c1, "📊", "10Y收益率水平", f"{m1['value']:.2f}", "%", m1["grade"], "0.15",
        "&lt;3.0 SAFE · 3-4 WATCH<br>4-5 WARNING · &gt;5 CRITICAL"
    )
    alert_metric_card(
        c2, "🚀", "3个月变化速率", f"{m2['value']:+.0f}", "bps", m2["grade"], "0.40",
        "历史P75=+50 · P90=+100<br>P95=+150bps<br>最关键预警指标"
    )
    alert_metric_card(
        c3, "📐", "30Y−10Y利差", f"{m3['value']:+.0f}", "bps", m3["grade"], "0.25",
        "&gt;20 SAFE · 0-20 WATCH<br>0~-30 WARNING · &lt;-30 CRITICAL"
    )
    alert_metric_card(
        c4, "🔗", "股债滚动相关性", f"{m4['value']:+.2f}", "r(12M)", m4["grade"], "0.20",
    ...
    with col_chart:
        fig_hist = build_alert_history_chart(y10, y30, sp500, "ALL")
    ...
    <div class="panel">
      ...
      <div style="margin-top:16px; padding-top:16px; border-top:1px solid rgba(255,255,255,0.06);
        ...
        历史回测中，2020年COVID崩盘期间综合分仅约45（WATCH），未能给出CRITICAL预警。
      </div>
    </div>
    """, unsafe_allow_html=True)
