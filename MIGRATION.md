...
- `components/transmission.py`：市场阶段与四节点传导链的共享展示组件；因子07的阶段文案已并入结论。
...
4. 因子01–07顶部综合评分均使用 `render_osci_card()` 和 SAFE／WATCH／WARNING／CRITICAL 四级状态；因子07的阶段仅作结论中的辅助说明。
5. 动态结论使用 `render_alert()`。
6. 数据来源和更新时间使用 `render_data_freshness()` 与 `render_footer()`。
7. 页面特有图表可以保留在页面内；重复出现的布局应进入 `components/`。
8. 因子01–07的说明区统一使用“检测逻辑／指标与历史证据／数据覆盖与来源”标签页。
9. 因子07历史事件评分取事件观察月末可得的当前版本数据；事后两项跌幅另以观察日前一交易日收盘价为基准，使用每日收盘价，不能混作事前预测。
